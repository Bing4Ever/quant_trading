...
class DataManager:
    ...
    def __init__(self, data_dir: str = None, db_url: str = None):
        ...
        """
        self.data_dir = Path(data_dir or config.get('market_data.data_directory', './data/'))
        self.db_url = db_url or config.get('market_data.database_url', 'sqlite:///market_data.db')
    ...
    def _needs_update(self, symbol: str, end_date: datetime = None) -> bool:
        ...
        if self.db_url.startswith('sqlite'):
            ...
            cursor = conn.execute(
                'SELECT last_update FROM data_updates WHERE symbol = ?',
                (symbol,)
            )
    ...
    def get_multiple_stocks(
        ...
        for symbol in symbols:
            results[symbol] = self.get_stock_data(symbol, start_date, end_date, force_update)
