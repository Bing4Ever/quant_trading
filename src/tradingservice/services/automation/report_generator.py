...
class ReportGenerator:
    ...
    def _collect_daily_data(self, target_date: date) -> Dict[str, Any]:
        ...
        try:
            ...
            if results:
                ...
                # 计算最佳策略
                best_result = max(results, key=lambda x: x.total_return or 0)
                data['best_strategy'] = {
                    'name': best_result.strategy_name or 'Unknown',
                    'return': best_result.total_return or 0,
                    'symbol': best_result.symbol or 'Unknown'
                }
        ...
        try:
            ...
            if results:
                ...
                for result in results:
                    ...
                    stats['count'] += 1
                    return_rate = result.total_return or 0
    ...
    def _calculate_monthly_summary(self, results: List["BacktestResult"]) -> Dict[str, Any]:
        """计算月度摘要统计"""
        total_returns = [r.total_return or 0 for r in results]
        sharpe_ratios = [r.sharpe_ratio or 0 for r in results]
        max_drawdowns = [r.max_drawdown or 0 for r in results]
    ...
    def _calculate_strategy_rankings(self, results: List["BacktestResult"]) -> List[Dict[str, Any]]:
        """计算策略排名"""
        ...
        for result in results:
            ...
            strategy_performance[strategy].append(result.total_return or 0)
    ...
    def _create_daily_report_content(self, data: Dict[str, Any], target_date: date) -> str:
        ...
        if data['backtest_results']:
            ...
            for result in data['backtest_results']:
                return_class = 'positive' if (result.total_return or 0) > 0 else 'negative'
                html_content += f"""
                    <tr>
                        <td>{result.strategy_name or 'Unknown'}</td>
                        <td>{result.symbol or 'Unknown'}</td>
                        <td class="{return_class}">{result.total_return or 0:.2%}</td>
                        <td>{result.sharpe_ratio or 0:.3f}</td>
                        <td>{result.max_drawdown or 0:.2%}</td>
                    </tr>
                """
