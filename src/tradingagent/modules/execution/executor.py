...
class OrderExecutor:
    ...
    def execute_signal(self, signal: TradingSignal) -> Optional[str]:
        ...
        try:
            ...
            if success:
                ...
                logger.info(f"订单提交成功: {order.order_id} - {signal.action} {signal.quantity} {signal.symbol}")
                return order.order_id
            else:
                self.failed_orders.append(order)
                logger.error(f"订单提交失败: {signal.action} {signal.quantity} {signal.symbol}")
                return None
    ...
    def get_order_statistics(self) -> Dict[str, Any]:
        ...
        Returns:
            Dict: 统计信息
        """
        total_orders = len(self.pending_orders) + len(self.completed_orders) + len(self.failed_orders)
