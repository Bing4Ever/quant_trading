...
class Trade:
    ...
    def close_trade(
        self, exit_date: datetime, exit_price: float, commission: float = 0.0
    ):
...
class BacktestEngine:
    ...
    def __init__(
        ...
        self.initial_capital = initial_capital or trading_config.get(
    ...
    def _close_all_positions(
        self, final_date: datetime, data_dict: Dict[str, pd.DataFrame]
    ):
    ...
    def _calculate_portfolio_value(
        self, date: datetime, data_dict: Dict[str, pd.DataFrame]
    ) -> float:
    ...
    def _calculate_performance_metrics(
        ...
        if benchmark_data is not None:
            ...
            risk_free_rate = (
                config.get("backtesting.risk_free_rate", 0.02) / 252
            )  # Daily
            alpha = annual_return - (
                risk_free_rate * 252
                + beta * (benchmark_returns.mean() * 252 - risk_free_rate * 252)
            )
            ...
            results.update(
                {
                    ...
                    "information_ratio": (
                        ...
                        * np.sqrt(252)
                        if (portfolio_returns - benchmark_returns).std() > 0
                        else 0
                    ),
                }
            )
