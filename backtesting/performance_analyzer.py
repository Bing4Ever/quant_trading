...
class PerformanceAnalyzer:
    ...
    def calculate_metrics(self) -> Dict[str, float]:
        ...
        metrics['win_rate'] = len(positive_returns) / len(self.returns) if len(self.returns) > 0 else 0
        ...
        # Additional metrics
        metrics['calmar_ratio'] = metrics['annualized_return'] / abs(metrics['max_drawdown']) if metrics['max_drawdown'] != 0 else float('inf')
    ...
    def plot_performance(self, figsize: Tuple[int, int] = (15, 10)) -> None:
        ...
        if len(self.returns) > 30:  # Only if we have enough data
            ...
            if not monthly_table.empty:
                sns.heatmap(monthly_table, annot=True, fmt='.2%', center=0, 
        ...
        axes[1, 1].axvline(self.returns.mean(), color='red', linestyle='--', 
